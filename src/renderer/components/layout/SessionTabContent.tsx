            ...
            <button
              onClick={() => {
                if (tab.projectId && tab.sessionId) {
                  void fetchSessionDetail(tab.projectId, tab.sessionId, tab.id, tab.contextId);
                }
              }}
    ...
    return (
      <div className="flex flex-1 items-center justify-center bg-claude-dark-bg">
        <div className="text-center">
          ...
          <p className="text-sm text-claude-dark-text-secondary">{t('layout.loadingSession')}</p>
        </div>
      </div>
    );
  }
