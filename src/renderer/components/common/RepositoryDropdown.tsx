...
export const RepositoryDropdown = ({
  ...
  const resolvedPlaceholder = placeholder ?? t('layout.selectRepository');
    ...
    <div ref={containerRef} className={`relative ${className}`}>
      ...
      >
        <span className="flex items-center gap-2">
          ...
          {isEmpty ? t('layout.noRepositoriesAvailable') : resolvedPlaceholder}
        </span>
        <ChevronDown
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          ...
          <span className="shrink-0 text-[10px] text-text-muted">
            {t(
              item.totalSessions === 1 ? 'layout.sessionCountOne' : 'layout.sessionCountOther',
  ...
  return (
    <div className="flex items-center gap-2 border-b border-border-subtle py-1.5">
      ...
      <button
        ...
        aria-label={t('layout.removeRepository')}
      >
